...
Choose one exactly as `reference/api.md` says under *Choosing a brand* — including when no brand is
marked default, which happens and is not a reason to publish an unbranded page. If the account has no
brands at all, carry on without one and say at the end that the page is in the default look.
